...
db.serialize(() => {
    ...
    db.run(`DELETE FROM portfolios WHERE chat_id = ?`, [TEST_USER_ID], function(err) {
        if (err) console.error("Error deleting from portfolios:", err);
        else console.log(`Deleted ${this.changes} rows from portfolios`);
    });
